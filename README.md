...
> Release note for v6.7.5: fixes the inverter derating calculator's grid power sign convention. The `sensor.grid` template (based on P1 meter) uses negative=import, positive=export, but the derating calculator expected the opposite. When feed-in was blocked and the site was importing from the grid, the calculator interpreted the import as export and wrongly curtailed solar production to zero. The fix inverts the grid power sign at the calculator entry point to match the actual sensor convention.
>
> Release note for v6.7.4: fixes the battery ON-hold logic to use the current effective price threshold (base × SOC multiplier) instead of a stale locked threshold captured at hold-entry. Previously, when a `battery_stable_threshold` relaxation deactivated mid-hold, the hold could keep charging ON at prices well above the real ceiling — wasting money. The hold now evaluates the live effective threshold every cycle, correctly releasing when the floor drops. SOC drift during the 5-minute hold window is negligible in practice, so the former locked-threshold guard is removed in favour of correctness under changing floors.
>
