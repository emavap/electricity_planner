...
def run_review(iteration: int) -> list:
    ...
    print("\n--- Category 1: Price Threshold Guard ---")
    test(
        "Guard: Emergency override (SOC 10%)",
        {
    ...
    test(
        "Guard: SOC relaxation (SOC 20%, price 0.18)",
        {
            "price_analysis": {
                "current_price": 0.18,
    ...
    test(
        "Battery: Near-full allows charging (SOC 85%)",
        {
            "price_analysis": {
                "current_price": 0.02,
                "price_threshold": 0.15,
    ...
    test(
        "Battery: No data, low price charges",
        {
            "price_analysis": {
                "current_price": 0.05,
                "price_threshold": 0.15,
    ...
    test(
        "Battery: No data, high price blocks",
        {
            "price_analysis": {"current_price": 0.50, "price_threshold": 0.15},
            "battery_analysis": {},
            "config": {},
    ...
    print("\n--- Category 3: Price Strategies ---")
    test(
        "VeryLowPrice: Excellent price (0.02)",
        {
            "price_analysis": {
                "current_price": 0.02,
                "price_threshold": 0.15,
    ...
    test(
        "Predictive: Wait for price drop",
        {
            "price_analysis": {
                "current_price": 0.10,
                "price_threshold": 0.15,
    ...
    test(
        "SOCBased: Medium SOC with solar waits",
        {
            "price_analysis": {
                "current_price": 0.10,
                "price_threshold": 0.15,
    ...
    test(
        "Edge: SOC exactly at emergency (15%)",
        {
            "price_analysis": {"current_price": 0.50, "price_threshold": 0.15},
            "battery_analysis": {"average_soc": 15},
            "config": {"emergency_soc_threshold": 15},
        },
        True,
        "emergency",
    ...
    test(
        "Edge: SOC just above emergency (16%)",
        {
            "price_analysis": {"current_price": 0.50, "price_threshold": 0.15},
            "battery_analysis": {"average_soc": 16},
            "config": {"emergency_soc_threshold": 15, "soc_buffer_target": 50},
        },
        False,
        "exceeds",
    ...
    test(
        "Boundary: SOC just below buffer (49%) - minimal relaxation",
        {
        ...
        False,
        "exceeds",
    )  # 0.16 > 0.151 (base 0.15 × 1.01 multiplier)
    # Note: DynamicPriceStrategy uses tighter dynamic threshold, not base threshold
    test(
        "Boundary: Price at base threshold but dynamic is tighter",
        {
            "price_analysis": {
                "current_price": 0.15,
                "price_threshold": 0.15,
        ...
        False,
        "",
    )  # Dynamic strategy finds tighter threshold
    test(
        "Boundary: Price just above threshold (no relaxation)",
        {
    ...
    test(
        "Boundary: Custom max SOC (80%)",
        {
            ...
            "battery_analysis": {"average_soc": 80, "max_soc_threshold": 80},
            "config": {},
            "power_analysis": {"significant_solar_surplus": False, "solar_surplus": 0},
        },
        False,
    ...
    print("\n--- Category 7: Strategy Interactions ---")
    test(
        "Interaction: Very low price overrides predictive wait",
        {
            "price_analysis": {
                "current_price": 0.02,
                "price_threshold": 0.15,
    ...
    test(
        "Interaction: Emergency overrides everything",
        {
            "price_analysis": {
                "current_price": 1.0,
                "price_threshold": 0.15,
    ...
    test(
        "Interaction: Buffer charging at relaxed price",
        {
            "price_analysis": {
                "current_price": 0.17,
                "price_threshold": 0.15,
