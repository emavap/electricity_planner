...
class NordpoolService:
    ...
    def clean_expired_cache(self) -> None:
        ...
        expired_keys = [
            key
            for key, (_, timestamp) in coordinator._nordpool_cache.items()
    ...
    async def fetch_prices(
        ...
        try:
            ...
            if day == "today":
                ...
                _LOGGER.error(
                    "Invalid day parameter: %s (expected 'today' or 'tomorrow')", day
                )
                return None
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    _LOGGER.warning(
                        "Nord Pool service call failed (invalid parameters) for %s: %s",
                        day,
                        err,
                    )
                    return None
                    ...
                    if attempt < max_retries - 1:
                        ...
                        _LOGGER.error(
                            "Nord Pool service timed out for %s after %d attempts: %s",
                            day,
                            max_retries,
                            err,
                        )
                    ...
                    if attempt < max_retries - 1:
                        ...
                        _LOGGER.error(
                            "Nord Pool service call failed for %s after %d attempts: %s",
                            day,
                            max_retries,
                            err,
                        )
            ...
            if response and isinstance(response, dict):
                ...
                total_entries = sum(
                    len(v) for v in response.values() if isinstance(v, list)
                )
                _LOGGER.debug(
                    "Fetched Nord Pool prices for %s (%s): %d entries across %d area(s)",
                    day,
                    target_date.isoformat(),
                    total_entries,
                    len(response),
                ...
                _LOGGER.warning(
                    "Nord Pool service returned unexpected format for %s", day
                )
                return None
            ...
            if isinstance(err, (KeyboardInterrupt, SystemExit)):
                raise
            _LOGGER.error(
                "Unexpected error fetching Nord Pool prices for %s: %s",
                day,
                err,
                exc_info=True,
            )
